...
<?php  
   ...
   $max_filesize = 2097152; // Максимальный размер файла в БАЙТАХ.
     ...
     $filename[$i] = date('d-m-y_h-i-s')."_".iconv('utf-8','windows-1251',  $f); // Получаем название файла (включая его расширение).
         $ext = strtolower( mb_substr($filename[$i], strpos($filename[$i],'.'), strlen($filename[$i])-1)); // Получаем расширение имени файла.
    ...
    if(!in_array($ext,$allowed_filetypes)) {
            die('<div class="error"> Некоторые файлы имеют недопустимый тип.<br> Разрешены только файлы изображений и pdf-документы.'.$ext.'</div><center><a href=123.html>Отмена</a></center>');
   ...
   if(filesize($f) > $max_filesize) {
      die('<div class="error"> Загружаемый файл слишком большой.</div><br><center><a href=123.html>Отмена</a></center>');
   ...
   // Проверяем, можно ли выгрузить в определенный путь, если нет, то DIE и информируем пользователя.
     ...
     // Перемещаем файл, если он прошел все проверки.
            $i=-1;
            echo "<center><a href='123.html' class='btn btn-primary'>Ещё</a></center>"; 
                foreach($_FILES['userfile']['tmp_name'] as $f){
                    $i++;
                
     if(move_uploaded_file($f,$upload_path[$i] . $file_strip[$i])) {
      echo '<div class="success">'.($file_strip[$i]) .' Файл успешно загружен. </div>'; // Получилось.
      echo '<script type="text/javascript">$(window.top.document.getElementById("adr")).append("|'.iconv('windows-1251','utf-8',$file_strip[$i]).'");$(window.top.document.getElementById("adr")).click(); </script>';
      resize($file_strip[$i]);
    } else {
