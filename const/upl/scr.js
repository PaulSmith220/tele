function makeUpload(){ //Добавляет в указанный див iframe,выбирающий файлы
    myframe = "<iframe style='display:none;' class='myload' src='frames/iframe.html'></iframe>";
    $("body").append(myframe);
...
function makeVisual(cont){ // Добавляет в область кнопку добавления файла и список файлов
   makeUpload()
      $("#"+cont).attr('id','files');
      $('body').append('<div id="r" style="display:none;">12</div><ul id="filelist"></ul>');
}
...
function fchange(fname){ // Проверяет,не пустое ли имя нового файла и добавляет его в список
    if(fname.trim()!=''){
    $('ul#filelist').append('<li style="cursor:pointer" title="'+file_col+'" id="flist_item"><a>'+fname+'</a> <img  style="display:none;" src="load.gif"/></li>');
 letitload(file_col);
 file_col++;
  }
}
...
$('#filelist #flist_item[title='+$("#r").attr('title')+']').css('color','green');
...
//$('#filelist #flist_item[title='+$("#r").attr('title')+']').remove();
 ...
 $('#filelist #flist_item[title='+$("#r").attr('title')+']').css('color','red');
$('#filelist #flist_item[title='+$("#r").attr('title')+']').children('img').hide(500);   
...
$(document).ready(function(){
   ...
   $("#filelist #flist_item").live('click',function(){
        $(".myload").attr('src','frames/iframe.html');
